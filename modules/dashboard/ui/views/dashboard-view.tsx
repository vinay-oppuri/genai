        ...
        <div className="min-h-screen bg-background px-6 py-10 text-gray-900 dark:text-gray-100 font-sans space-y-10 transition-all">
            <header className="text-center space-y-2">
                <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight">
                    Welcome back,{" "}
                    <span className="text-primary dark:text-green-500">
                        {session?.user.name || "-----"}
                    </span>
                </h1>
                ...
                {isFreeUser && (<p className="md:hidden text-muted-foreground text-shadow-md">You are a <span className="text-green-600 text-lg">Free</span> User</p>)}
            </header>
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {testData.map(({ title, value, max }, i) => (
                    <Card
                        key={i}
                        className="bg-muted border backdrop-blur-xl shadow-lg shadow-primary/10 hover:scale-101 transition-all duration-300"
                    >
                        <CardHeader>
            ...
            <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card className="bg-muted border backdrop-blur-xl shadow-lg shadow-primary/10 transition-all duration-300 hover:scale-101">
                    <CardHeader>
